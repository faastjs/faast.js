...
import { FunctionCallSerialized, createErrorResponse } from "../wrapper";
...
function processIncomingQueueMessage(m: SQS.Message): ReceivableMessage | void {
    ...
    // provides.
    const kind = sqsMessageAttribute(m, KIND_ATTR) as ReceivableKind | undefined;
    if (kind === undefined) {
        ...
        if (body.responseContext.functionError) {
            ...
            const sCall: FunctionCallSerialized = deserializeMessage(record.Sns.Message);
            const error = processAwsErrorMessage(
                (body.responsePayload as LambdaDestinationError).errorMessage
            );
